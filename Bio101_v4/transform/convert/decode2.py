...
def convert_to_file(npath, token):
	...
	isnt2bit  = os.path.join(work_dir, 'isnt2bitos')	# for mac os system
	...
	try:
		...
		# call C-language module to decode the information
		return stem
	except:
		return ''
...
class decoding2(object):
	def __init__(self, fname, token):
		self.Fi = {}
		seq = file_seq(fname)
		# get the fragment which need to be decode
		if seq:
			self.fragments(seq)
	...
	# grab the index and information DNA
	...
	# order the DNAs base on index, reverse the odd DNA sequence
